...
def api_neon_metrics():
    ...
    try:
        ...
        if not neon_api_key:
            return jsonify({
                'success': False,
                'error': 'NEON_API_KEY no configurada',
                'message': 'Configure la variable de entorno NEON_API_KEY en Vercel'
            }), 200  # 200 para que no crashee
        ...
        if not projects:
            return jsonify({
                'success': False,
                'error': 'No se pudo conectar con Neon API',
                'message': 'Verifique que la API key sea válida'
            }), 200
